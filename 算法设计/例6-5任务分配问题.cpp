...
struct NodeType		//队列结点类型
{
	...
	int cost;			//【已经分配任务】所需要的成本
	int lb;			//下界
	bool operator<(const NodeType& s) const	//重载<关系函数（想要使用优先队列 就要先重载关系函数！）
	{
		return lb > s.lb;
	}
};
...
void bound(NodeType& e)	     //求结点e的限界值（剩余未分配人员可能的最小分配方法）	
{
	int minsum = 0;
	for (int i1 = e.i + 1; i1 <= n; i1++) //求c[e.i+1..n]行中最小元素和
	{
		...
		for (int j1 = 1; j1 <= n; j1++) {
			if (e.worker[j1] == false && c[i1][j1] < minc)//各列中仅仅考虑没有分配的任务
				minc = c[i1][j1];
		}
		minsum += minc;
	}
