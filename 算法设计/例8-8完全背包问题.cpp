...
//另外设置二维数组fk，其中fk[i][j]存放dp[i][j]得到最大值时物品i挑选的【件数】。
//状态转移方程：
//dp[i][j] = MAX{ dp[i - 1][j - k * w[i]] + k * v[i] }  当dp[i][j] <dp[i - 1][j - k * w[i]] + k * v[i]（k * w[i]≤j），k是不断变化的
//fk[i][j] = k;			    	    物品i取k件
//
//这样，dp[n][W]便是最大重量为W、考虑所有n个物品（同一物品允许多次选择）后得到的背包最大总价值，即问题的最优结果。
