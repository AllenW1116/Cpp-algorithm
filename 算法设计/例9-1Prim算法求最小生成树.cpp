...
void Prim(MGraph g, int v)	   //Prim算法 v为源点
{
	...
	for (j = 0; j < g.n; j++)	   //给初始化lowcost和closest数组，初始时集合U只包含源点v
	{
		lowcost[j] = g.edges[v][j];
		closest[j] = v; //其实初始化成啥都不影响
	}
	for (i = 1; i < g.n; i++)		//找出(n-1)个顶点
	{
		mincost = INF;
		for (j = 0; j < g.n; j++)		//在(V-U)中找出离U最近的顶点k
			if (lowcost[j] != 0 && lowcost[j] < mincost) //lowcost=0，说明顶点已经在U中
			{
				mincost = lowcost[j];
		...
		for (j = 0; j < g.n; j++)		//修改数组lowcost和closest：由于U中新加入了k节点，V-U中的节点距离U的最近距离可能就是它和k节点的距离
			if (g.edges[k][j] != 0 && g.edges[k][j] < lowcost[j]) //更新路径
			{
